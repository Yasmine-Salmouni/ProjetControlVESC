...
/*float ScreenDisplay::getUserPower() {
 ...
 float ScreenDisplay::getUserPower() {
    // Vide le buffer UART avant d'envoyer la commande
    while (USART2->SR & USART_SR_RXNE) {
    ...
    if (!headerFound) {
        sendText("err", "No pow data");
        return -1.0f;
    }
    return atof(buffer);  // Convertit la chaîne en float (W)
}
...
/*float ScreenDisplay::getUserTorque() {
 ...
 float ScreenDisplay::getUserTorque() {
     ...
     while (HAL_GetTick() - startTime < 20) {
         if (!(USART2->SR & USART_SR_RXNE)) {
             continue;
         }
         uint8_t byte = static_cast<uint8_t>(USART2->DR & 0xFF);
...
ControlMode ScreenDisplay::getMode() {
    // Vider le buffer UART avant d'envoyer la commande
    while (USART2->SR & USART_SR_RXNE) {
        volatile uint8_t dummy = USART2->DR;
    }
    ...
    if (octetCount == MAX_RESPONSE) {
        ...
        switch (lastModeValue) {
            ...
            case 4: last_valid_mode = ControlMode::LINEAR; break;
...
float ScreenDisplay::getFatigueLabel() {
      sendCommand("get fatiguelabel.txt");  // Demande la valeur du champ texte "tor"
